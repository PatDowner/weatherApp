let searches = ['Austin', 'Chicago', 'New York', 'Boston', 'Orlando', 'San Francisco', 'Seattle', 'Denver', 'Atlanta']
// searches = []
...
if (searches.length = 0) {
  console.log('nada')
  document.getElementById('recentSearches').parentNode.className = "hide"
} else {
  console.log('hello')
  for (let i = searches.length; i = searches.length - 9; i--) {

    // make new city an link item
    let newCity = document.createElement('a')

    // set classes for the link to become a list item for recentSearches
    newCity.className = 'list-group-item list-group-item-action'

    // sets text to appear in the link list item
    newCity.innerHTML = `
    <a href = "https://api.openweathermap.org/data/2.5/weather?q=${newCity[i]}&units=imperial&appid=1dd25ac798a84daed3b612ef4b3c9a3e>${newCity[i]}</a>
    `

    document.getElementById('recentSearches').append(newCity)
  }
}
...
if (searches.length = 0) {
  document.getElementById('list').innerHTML = `<div><h5 class="card-header text-center">No Past Searches</h5></div>`
} else {
  console.log('hello')
  for (let i = searches.length; i = searches.length - 9; i--) {
    ...
    // set link address of city name
    newCity.href = `https://api.openweathermap.org/data/2.5/weather?q=${newCity[i]}&units=imperial&appid=1dd25ac798a84daed3b612ef4b3c9a3e`

    // sets text to appear in the link list item
    newCity.textContent = newCity[i]
